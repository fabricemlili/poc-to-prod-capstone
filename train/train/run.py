...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    dataset = LocalTextCategorizationDataset(dataset_path, train_conf["batch_size"], min_samples_per_label = train_conf["min_samples_per_label"], preprocess_text=embed)
    ...
    model.add(Dense(dataset.get_num_labels()))

    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    model.summary()
    # TODO: CODE HERE
    # model fit using data sequences
    train_history = model.fit(dataset.get_train_sequence(), validation_data=dataset.get_test_sequence(), epochs=train_conf["epochs"])
    ...
    json_params = json.dumps(train_conf)
    with open(f'{artefacts_path}/params.json', 'w') as file:
        file.write(json_params)

    # TODO: CODE HERE
    # save labels index in artefacts_path/labels_index.json
    json_labels_index = json.dumps(dataset.get_label_to_index_map())
    with open(f'{artefacts_path}/labels_index.json', 'w') as file:
        file.write(json_labels_index)
