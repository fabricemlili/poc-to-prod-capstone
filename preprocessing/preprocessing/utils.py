...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def __init__(self, filename, batch_size,
        ...
        self._label_list = self._dataset["tag_name"].values.tolist()

        y = self.to_indexes(self._label_list)
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        columns = ["post_id", "tag_name", "tag_id", "tag_position", "title"]
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                return df[df['tag_name'].map(df['tag_name'].value_counts())>=x]
        ...
        dataframe = (df.pipe(filter_tag_position(0))
